...
export function Voters (props) {
    ...
    async function updateState () {
        let user_has_voted = await getHasVoted({"user_id": loginInfo.userID, "question": props.id})
        let net_votes = await getHasVoted({"question": props.id})
    ...
    async function Upvote (e) {
        if (canVote) {
            postHasVoted({"user": loginInfo.userID, "username": loginInfo.username, "question": props.id, "vote_type": "up"})
            setCanVote(false)
    ...
    async function Downvote (e) {
        if (canVote) {
            postHasVoted({"user": loginInfo.userID, "username": loginInfo.username, "question": props.id, "vote_type": "down"})
            setCanVote(false)
            setNumVotes(numVotes - 1)
        }
    }
